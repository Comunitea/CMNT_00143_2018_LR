...
var StockProvider = /** @class */ (function () {
    ...
    StockProvider.prototype.create_new_package_from_move = function (move_id, partner_id) {
        var self = this;
        var model;
        var values = {
        ...
        var promise = new Promise(function (resolve, reject) {
            self.odooCon.execute(model, 'create_new_package_from_move', values).then(function (done) {
                resolve(done);
            })
                .catch(function (err) {
                reject(false);
    ...
    StockProvider.prototype.get_apk_info_full = function (values) {
        var self = this;
        var model;
    ...
    StockProvider.prototype.set_move_shipping_type = function (move_id, values) {
        var self = this;
        var model;
        model = 'stock.move.line';
        var promise = new Promise(function (resolve, reject) {
    ...
    StockProvider.prototype.set_move_line_info = function (move_id, values) {
        return this.set_move_shipping_type(move_id, values);
        var self = this;
        var model = 'stock.move.line';
        var promise = new Promise(function (resolve, reject) {
            self.odooCon.update_lines(model, 'write', values, move_id).then(function (sp_ids) {
                for (var sm_id in sp_ids) {
                    sp_ids[sm_id]['model'] = model;
                }
                resolve(sp_ids);
...
var StockMoveListPage = /** @class */ (function () {
    function StockMoveListPage(alertCtrl, actionSheetCtrl, dragulaService, navCtrl, navParams, viewCtrl, storage, stockInfo, changeDetectorRef) {
        ...
        this.subs.add(this.dragulaService.dropModel()
            ...
            if (target.id == "pkgs" && source.id == "lines") {
                _this.update_packages('stock.move.line', parseInt(el.id), false, "new");
            }
            else if (target.id == "pkgs_info" && source.id == "lines") {
                _this.update_packages('stock.move.line', parseInt(el.id));
            }
            else if (target.id == "pkgs_info" && source.id == "arrival_pkgs") {
                _this.update_packages('stock.quant.package', parseInt(el.id));
            }
        }));
        this.subs.add(this.dragulaService.removeModel()
            ...
            if (source.id == "pkgs_info") {
                _this.update_packages('stock.quant.package', parseInt(el.id), false, "unlink");
            }
    ...
    StockMoveListPage.prototype.act_move_filter = function () {
        for (var move_n in this.full_stock_moves) {
            var move = this.full_stock_moves[move_n];
            var active = ((move['shipping_type'] && move['shipping_type'] == this.current_shipping_type) || this.current_shipping_type == 'all')
                &&
                    ((this.current_list_shown == 'filtered_unassigned' && !move['result_package_id']) || (this.current_list_shown == 'filtered_assigned' && move['result_package_id']) || (this.current_list_shown == 'move_list'));
            this.full_stock_moves[move_n]['active'] = active;
        }
    };
    ...
    StockMoveListPage.prototype.get_partner_move_lines_apk = function (partner_id, current_selected_pkg) {
        ...
        this.stockInfo.get_apk_info_full(values).then(function (lines) {
    ...
    StockMoveListPage.prototype.update_packages = function (model, move_ids, package_id, action, partner_id) {
        var _this = this;
        if (package_id === void 0) { package_id = this.current_selected_pkg; }
        if (action === void 0) { action = false; }
        if (partner_id === void 0) { partner_id = false; }
        console.log(partner_id);
        this.stockInfo.update_object(model, action, move_ids, false, package_id, partner_id).then(function (resultado) {
            if (resultado[0]) {
    ...
    // Shipping type
    StockMoveListPage.prototype.show_shipping_options_package = function (model, package_id) {
        var _this = this;
        this.stockInfo.get_package_info(package_id).then(function (lineas) {
    ...
    StockMoveListPage.prototype.show_shipping_options_line = function (model, line_id) {
        var _this = this;
    ...
    StockMoveListPage.prototype.reload_with_data = function (current_selected_partner, current_selected_pkg, current_shipping_type) {
        ...
        var val = {
            'current_selected_partner': current_selected_partner,
            'current_selected_pkg': current_selected_pkg,
            'current_shipping_type': current_shipping_type,
            'current_list_shown': this.current_list_shown
        };
        this.navCtrl.setRoot(StockMoveListPage_1, val);
    };
    // Checkboxes selection
    StockMoveListPage.prototype.multipleSelection = function (event) {
        var checkeableItems = this.full_stock_moves.filter(function (x) { return x['active'] === true; });
    ...
    StockMoveListPage.prototype.update_shipping_type = function (id, type, values, role) {
        var _this = this;
        if (type == 'line') {
            this.stockInfo.set_move_line_info(id, values).then(function (resultado) {
