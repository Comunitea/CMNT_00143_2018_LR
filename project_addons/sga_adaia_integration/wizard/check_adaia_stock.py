...
class WizCheckAdaiaStock(models.TransientModel):

    _name = "wiz.check.adaia.stock"
    ...
    def regularize(self):
        ...
        string = "RESUMEN de generacion del fichero: {}" \
        ...
        sga_file_obj.write_log(string, log_name, False)
        return new_sga_file


    def get_product_waiting_sga_confirm(self, domain= []):

        domain += [('state', '=', 'assigned'), ('sga_state', '=', 'pending')]
