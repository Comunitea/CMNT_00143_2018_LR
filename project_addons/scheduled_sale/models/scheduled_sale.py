...
class ScheduleSalePeriod(models.Model):
    ...
    first_order_date = fields.Date('First order date', required=True)
...
class ScheduleSale(models.Model):
    ...
    pricelist_id = fields.Many2one('product.pricelist', string='Active Pricelist', compute="get_active_pricelist",
                                   help="Pricelist for scheduled sale.")
    first_order_pricelist_id = fields.Many2one('product.pricelist', string='1º order pricelist', required=True, readonly=False,
                                   help="Pricelist for scheduled sale.",
                                   states={'confirm': [('readonly', True)], 'done': [('readonly', True)]})
    scheduled_pricelist_id = fields.Many2one('product.pricelist', string='Schedule Pricelist', required=True, readonly=False,
                                               help="Pricelist for scheduled sale.",
                                               states={'confirm': [('readonly', True)], 'done': [('readonly', True)]})
    ...
    first_order = fields.Boolean("First order")

    first_order_date = fields.Date('First order date')
    ...
    def get_active_pricelist(self):
        for sch in self:
            date_from = fields.Date.from_string(sch.from_date)
            #to_date = fields.Date.from_string(sch.to_date)
            #first_order_date = fields.Date.from_string(sch.first_order_date)
            now = fields.Date.today()
            if date_from:

                if now < date_from:
                    sch.pricelist_id = sch.first_order_pricelist_id
                else:
                    sch.pricelist_id = sch.scheduled_pricelist_id
    ...
    def onchange_period_id(self):
        if self.period_id:
            self.from_date = self.period_id.from_date
            self.to_date = self.period_id.to_date
            self.first_order_date = self.period_id.first_order_date

    def apply_dates(self):
        if not self.from_date or not self.to_date or not self.first_order_date:
            raise ValidationError (_('Check scheduled dates'))
        new_dates = {'date_start': self.from_date, 'date_end': self.to_date}
        self.scheduled_pricelist_id.item_ids.write(new_dates)

        new_dates = {'date_start': self.first_order_date, 'date_end': self.from_date}
        self.first_order_pricelist_id.item_ids.write(new_dates)


    @api.multi
    def confirm_schedule(self):
        for scheduled in self:
            scheduled.state = "confirm"
            scheduled.apply_dates()
