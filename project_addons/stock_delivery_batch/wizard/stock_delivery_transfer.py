...
class StockDeliveryTransfer(models.TransientModel):
    _name = 'stock.delivery.transfer'
    _inherit = 'stock.immediate.transfer'
    _description = 'Delivery Transfer'

    pick_to_backorder_ids = fields.Many2many('stock.picking', help='Picking to backorder')

    @api.multi
    def process(self):
        backorder_wizard_dict = super(StockDeliveryTransfer, self).process()
